...
function normalizeChannelPageVideo(html) {
  const videoId = html.match(/"videoId":"([^"]+)"/)?.[1]

  if (!videoId) {
    throw new Error('YouTube channel page did not include a video ID')
  }

  const videoChunkStart = html.indexOf(`"videoId":"${videoId}"`)
  const videoChunk = html.slice(videoChunkStart, videoChunkStart + 9000)
  const title =
    videoChunk.match(/"title":\{"content":"([^"]+)"/)?.[1] ||
    html.match(/"title":\{"content":"([^"]+)"/)?.[1] ||
    fallbackVideo.title
...
async function fetchLatestVideoFromChannelPage() {
  const html = await fetchText(`https://www.youtube.com/@${youtubeHandle}/videos`)
  return normalizeChannelPageVideo(html)
}
...
export async function fetchLatestVideo() {
  ...
  try {
    ...
    cachedVideo = await fetchLatestVideoFromChannelPage()
  }
