...
function extractJsonScript(html) {
  ...
  if (!match?.[1]) {
    throw new Error('X syndication page did not include timeline data')
  }
...
async function fetchLatestTweetsFromSyndication() {
  const html = await fetchText(
    `https://syndication.twitter.com/srv/timeline-profile/screen-name/${xScreenName}`,
  )
  const data = extractJsonScript(html)
  const entries =
    data?.props?.pageProps?.timeline?.entries?.filter(
      (entry) => entry?.type === 'tweet' && entry?.content?.tweet,
    ) || []

  return entries
    .map((entry) => entry.content.tweet)
    .filter((tweet) => tweet?.user?.screen_name === xScreenName)
    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
    .slice(0, 6)
    .map((tweet) => ({
      id: tweet.id_str,
      text: cleanTweetText(tweet.full_text || tweet.text),
      createdAt: tweet.created_at,
      url: `https://x.com/${xScreenName}/status/${tweet.id_str}`,
    }))
}
...
export async function fetchLatestTweets() {
  ...
  try {
    tweets = await fetchLatestTweetsFromRss()
  } catch (rssError) {
    console.error(rssError)
    tweets = await fetchLatestTweetsFromSyndication()
  }
